...
- **종목 자동 선정**: 거래대금·거래량·상승률 랭킹 → 경고/유형/가격/등락률/거래대금/RVOL/틱비용 필터 → 점수화 (Stocks in Play) → 상위 종목을 자동으로 골라 봇 생성. 사용자가 종목을 고를 필요 없음
- **종목별 봇**: 종목마다 독립 운용 (전략·손절·분할익절·트레일링·타임스탑·목표수익률). 기본값은 [설정 → 자동 운용]의 프리셋, 실행 중인 봇은 [설정 변경]으로 개별 수정
- **종가매매**: 14:40 부터 스캐너가 종가매매 후보 모드로 자동 전환, 14:50 단타 정리 후 조건(✔/✖)을 모두 통과한 종목으로 15:00~15:19 매수 → 익일 매도. 봇·모의계좌는 재시작해도 이어서 관리
...
dotnet test --project tests\TossTrading.Tests       # 테스트 (xunit.v3)
```
...
2. **[🤖 자동 운용]** 이 켜져 있으면(기본) 스캐너 상위 종목으로 봇이 자동으로 만들어지고 매매·정리까지 진행됩니다.
   봇 목록에서는 [청산] / [정지(청산)] / [설정 변경]만 하면 됩니다. 손절·익절 등 기본값은 ⚙ 설정 → 자동 운용 → 프리셋 [편집]
3. **📈 백테스트** 로 과거 기간 성과(수익률·수익금·MDD)를 확인
